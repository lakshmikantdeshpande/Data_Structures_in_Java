...
public class Percolation {
	...
	public void open(int i, int j) {
		...
		if (i != 1 && isOpen(i - 1, j))   // if the row is not the first one, call union method
			connect(mapIndices(i - 1, j), cell);
		else if (i == 1)
			connect(cell, size*size); // If it's a top row, connect the cell to the virtual top root
		
		if (i != size && isOpen(i+1, j))
			connect(mapIndices(i+1, j), cell);
		else if (i == size)
			connect(cell, size*size + 1); // If it's a bottom row, connect the cell to the virtual bottom root
		
		if (j != 1 && isOpen(i, j-1))
			connect(mapIndices(i, j-1), cell);    // if the cell is not in the leftmost column
		
		if (j != size && isOpen(i, j+1))
			connect(mapIndices(i, j+1), cell);    // if the cell is not in the rightmost column
	}
